...
create_box_plot <- function(df, model_type=NULL) {
  ...
  p <- ggplot(df_long, aes(x = fct_relevel(Metric, "Places", "Transitions", "Arcs"), y = Count)) +
    ...
    scale_y_log10(breaks = c(1,10,100,1e3,1e4,1e5,1e6,1e7),  # Manual breaks
                  ...
                  limits = c(1, 1e7)) +  # Set the y-axis limits to ensure uniform scaling) 
    theme_minimal() +
    xlab(NULL) +
    ylab(NULL) +
    theme(
      axis.title = element_text(size = 35, face = "bold"),   # Increase size & make axis title bold
      axis.text = element_text(size = 35, face = "bold"),    # Increase size & make axis text bold
      legend.title = element_text(size = 35, face = "bold"), # Increase size & make legend title bold
      legend.text = element_text(size = 35, face = "bold")   # Increase size & make legend text bold
    )
  
  ggsave(filename = paste0("box_plot_", model_type, ".pdf"), plot = p, device = "pdf", width = 8, height = 7)
}


# Function to create box plot
create_box_plot2 <- function(df, model_type=NULL) {
  if (!is.null(model_type)) {
    df <- df %>% filter(ModelTypeUpdated == model_type)
  } else {
    model_type <- "All"
